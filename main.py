...
if plugin.get_setting('english', str) == 'true':
    headers = {
...
def ls_list(url, type, export):
    ...
    for list_item in list_items:
        ...
        img_match = re.search(r'"(https://images-na.ssl-images-amazon.com/images.*?)"', list_item,
                              flags=(re.DOTALL | re.MULTILINE))
        if img_match:
        ...
        title_match = re.search(r'<a.*?href="/title/(tt[0-9]*)/\?ref_=ttls_li_tt.*?>(.*?)</a>', list_item,
                                flags=(re.DOTALL | re.MULTILINE))
        if title_match:
            imdbID = title_match.group(1)
            title = title_match.group(2)
        temp_data['title'] = html_parser.unescape(title)

        # <span class="lister-item-year text-muted unbold">(2017)</span>
        type = 'featureFilm'
        title_match = re.search(r'<span class="lister-item-year.*?>\((.*?)\)</span>', list_item,
                                flags=(re.DOTALL | re.MULTILINE))
        if title_match:
        ...
        rating = ''
        votes = ''
        rating_match = re.search(r'title="Users rated this (.+?)/10 \((.+?) votes\)', list_item,
                                 flags=(re.DOTALL | re.MULTILINE))
        if rating_match:
        ...
        plot = ''
        runtime = 0
        plot_match = re.search(r'<div class="item_description">(.*?)<span>\((.*?) mins\.\)</span></div>', list_item,
                               flags=(re.DOTALL | re.MULTILINE))
        if plot_match:
...
def movie_search(url, type, export):
    ...
    while url:
        ...
        for match in matches:
            ...
            title = match[1]
            year = match[2]
            add_to_library(imdb_id, type, urllib.parse.quote_plus(title), year)
        match = re.search('<a href="(.*?)&ref_=adv_nxt"', html)
        if match:
            url = "http://www.imdb.com/search/title" + match.group(1)
        count = count + 1
        if count >= int(plugin.get_setting('search.pages', str)):
            break
...
def list_titles(imdb_ids, order, list_type, export):
    ...
    for imdb_id in order:
        ...
        try:
            plot = imdb_data['plot']
            temp_data['plot'] = html_parser.unescape(plot.decode('utf-8'))
        except:
            pass
        temp_data['cast'] = []
        try:
...
def add_to_trakt_watchlist(type, imdb_id, title):
    ...
    if not plugin.get_setting('authorization', str):
        if not authenticate():
            return
    authorization = json.loads(plugin.get_setting('authorization', str))
...
def make_list(imdb_ids, order, list_type, export):
    ...
    items = []
    for imdb_id in order:
        if plugin.get_setting('hide_duplicates', str) == "true" and existInKodiLibrary(imdb_id):
            continue
        imdb_data = imdb_ids[imdb_id]
        ...
        if type == "series":  # TODO episode
            ...
            meta_url = "plugin://plugin.video.imdb.watchlists/meta_tvdb/%s/%s" % (
                imdb_id, urllib.parse.quote_plus(title.encode("utf8")))
        elif type == "featureFilm":
            trakt_type = 'movies'
            meta_url = 'plugin://%s/movies/play/imdb/%s/library' % (
                plugin.get_setting('catchup.plugin', str).lower(), imdb_id)
        context_items = []
        try:
            if xbmcaddon.Addon('plugin.program.super.favourites'):
                context_items.append(
                    ('iSearch', 'ActivateWindow(%d,"plugin://%s/?mode=%d&keyword=%s",return)' % (
                        10025, 'plugin.program.super.favourites', 0, urllib.parse.quote_plus(title.encode("utf8")))))
        except:
            pass
        context_items.append(
        ...
        context_items.append(
            ('Delete from Library',
             'XBMC.RunPlugin(%s)' % (plugin.url_for('delete_from_library', imdb_id=imdb_id, type=type))))
        context_items.append(('Add to Trakt Watchlist', 'XBMC.RunPlugin(%s)' % (
            plugin.url_for('add_to_trakt_watchlist', type=trakt_type, imdb_id=imdb_id, title=title.encode("utf8")))))
        context_items.append(('Add to Trakt Collection', 'XBMC.RunPlugin(%s)' % (
            plugin.url_for('add_to_trakt_collection', type=trakt_type, imdb_id=imdb_id, title=title.encode("utf8")))))
        try:
            if type == 'featureFilm' and xbmcaddon.Addon('plugin.video.couchpotato_manager'):
                context_items.append(
                    ('Add to Couch Potato',
                     "XBMC.RunPlugin(plugin://plugin.video.couchpotato_manager/movies/add-by-id/%s)" % (imdb_id)))
        except:
            pass
        try:
                ...
                context_items.append(
                    ('Add to Sickrage',
                     "XBMC.RunPlugin(plugin://plugin.video.sickrage?action=addshow&&show_name=%s)" % (
                         urllib.parse.quote_plus(title.encode("utf8")))))
        except:
            pass
...
def existInKodiLibrary(id, season="1", episode="1"):
    ...
    if 'tt' in id:
        ...
        if id + "-" + str(season) + "-" + str(episode) in showDict:
            result = True
    return result
...
def add_to_library(imdb_id, type, title, year):
    ...
    if type == "series":
        try:
            xbmcvfs.mkdirs('special://profile/addon_data/plugin.video.imdb.watchlists/TV/%s' % imdb_id)
        except:
            pass
        ...
        if plugin.get_setting('duplicates') == "false" and existInKodiLibrary(imdb_id):
            ...
            f = xbmcvfs.File('special://profile/addon_data/plugin.video.imdb.watchlists/Movies/%s.strm' % (imdb_id),
                             "wb")
            ...
            f.close()
            f = xbmcvfs.File('special://profile/addon_data/plugin.video.imdb.watchlists/Movies/%s.nfo' % (imdb_id),
                             "wb")
...
def update_tv():
    ...
    xbmcvfs.mkdirs('special://profile/addon_data/plugin.video.imdb.watchlists/TV')
    try:
        last_run = datetime.fromtimestamp(time.mktime(
            time.strptime(plugin.get_setting('update_tv_time').encode('utf-8', 'replace'), "%Y-%m-%d %H:%M:%S")))
    except:
    ...
    if update_all == False:
        ...
        data = results.text
        try:
            to_zip = zipfile.ZipFile(io.StringIO(data))
            z = to_zip.open('updates_%s.xml' % period)
            xml = z.read().decode('UTF-8')
        except:
            return
        match = re.compile(
            r'<Series><id>(.*?)</id><time>(.*?)</time></Series>',
            flags=(re.DOTALL | re.MULTILINE)
        ).findall(xml)
...
def update_tv_series(imdb_id):
    ...
    data = results.text
    try:
        zip = zipfile.ZipFile(io.StringIO(data))
        z = zip.open('en.xml')
        xml = z.read().decode('UTF-8')
    except:
        return
                    ...
                    if not since or (aired > (today - since)):
                        if plugin.get_setting('duplicates') == "false" and existInKodiLibrary(id, season, episode):
                            ...
                            if tv_library_url == "true" and meta_url:
                                ...
                                meta_url = meta_url.replace("%W", urllib.parse.quote_plus(series_name))
                                meta_url = meta_url.replace("%S", season)
                                meta_url = meta_url.replace("%E", episode)
                                meta_url = meta_url.replace("%V", tvdb_id)
                            else:
                                meta_url = "plugin://%s/tv/play/%s/%d/%d/library" % (
                                    plugin.get_setting('catchup.plugin').lower(), tvdb_id, int(season), int(episode))
                            f = xbmcvfs.File(
                                'special://profile/addon_data/plugin.video.imdb.watchlists/TV/%s/S%02dE%02d.strm' % (
                                    imdb_id, int(season), int(episode)), "wb")
                            f.write(meta_url.encode("utf8"))
                            f.close()
...
def nuke():
    ...
    if not ok:
        return
    for root in ['special://profile/addon_data/plugin.video.imdb.watchlists/TV',
...
def category(type):
    ...
    ur_sort = ['list_order', 'alpha', 'user_rating', 'your_rating', 'moviemeter', 'num_votes', 'release_date',
               'date_added']
    ur_order = ['asc', 'desc']
    ls_sort = ['listorian', 'title', 'user_rating', 'your_ratings', 'moviemeter', 'num_votes', 'release_date_us',
               'created']
    ls_order = ['asc', 'desc']
    for watchlist in sorted(watchlists):
        ...
        if 'list/ls' in url:
            ...
            if sort:
                match = re.search(r'/(ur[0-9]*)', url)
                ur = match.group(1)
                url = "http://www.imdb.com/user/%s/watchlist?sort=%s%%2C%s" % (
                    ur, ur_sort[int(sort)], ur_order[int(order)])
        context_items = []
        context_items.append(
            ('Add to Library', 'XBMC.RunPlugin(%s)' % (plugin.url_for(route, url=url, type=type, export=True))))
        context_items.append(
            ('Remove Watchlist', 'XBMC.RunPlugin(%s)' % (plugin.url_for('remove_watchlist', watchlist=watchlist))))
        context_items = context_items + main_context_items
        if watchlist in library_watchlists:
            label = "%s (subscribed)" % watchlist
            context_items.append(
                ('Unsubscribe', 'XBMC.RunPlugin(%s)' % (plugin.url_for(unsubscribe_watchlist, watchlist=watchlist))))
        else:
            label = watchlist
            context_items.append(
                ('Subscribe', 'XBMC.RunPlugin(%s)' % (plugin.url_for(subscribe_watchlist, watchlist=watchlist))))
        items.append(
            {
                'label': label,
